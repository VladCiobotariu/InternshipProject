...
import com.ozius.internship.project.domain.exception.IllegalItemException;
...
class SellerReviewEntityTests extends JpaBaseEntity {
    ...
    @Test
    void review_is_added_correctly() {
        //----Act
        ...
        assertThat(review.getPublishDate().isEqual(LocalDate.now())).isTrue();

    }

    @Test
    void review_is_updated() {
        //----Arrange
        Seller savedSeller = doTransaction(em -> {
            Buyer buyer1 = em.merge(TestDataCreator.Buyers.buyer1);
            Product product = em.merge(product1);
    ...
    @Test
    void review_added_wrong_product_throws_exception(){
        //----Act
    ...
    @Test
    void review_added_wrong_rating_throws_exception() {
        // ----Act
        Exception exception = doTransaction(em -> {
            Buyer buyer1 = em.merge(TestDataCreator.Buyers.buyer1);
    ...
    @Test
    void review_updated_wrong_from_rating_throws_exception() {
        //----Arrange
        Seller savedSeller = doTransaction(em -> {
            Buyer buyer1 = em.merge(TestDataCreator.Buyers.buyer1);
            Product product = em.merge(product1);
